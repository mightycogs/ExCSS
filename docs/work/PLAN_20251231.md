...
- Gradients: `LinearGradient`, `RadialGradient`, `GradientStop`
...
│                            ExCSS                                     │
│  ┌─────────────┐    ┌──────────────────┐    ┌────────────────────┐  │
│  │   Parser    │───▶│   Converters     │───▶│  IStyleValue       │  │
│  │  (tokens)   │    │  (existing, 65+) │    │  (NEW public API)  │  │
│  │             │    │                  │    │                    │  │
│  │ "10px"      │    │ LengthConverter  │    │ Length struct      │  │
│  │ "#ff0"      │    │ ColorConverter   │    │ Color struct       │  │
│  │ "var(--x)"  │    │ VarConverter     │    │ VarValue           │  │
│  └─────────────┘    └──────────────────┘    └────────────────────┘  │
│                              │                        │             │
│                              ▼                        ▼             │
│  ┌─────────────────────────────────────────────────────────────────┐│
...
│                         MightyUI                                     │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ CSSParser.GetStylesFor() → ComputedStyle with IStyleValue       ││
│  │ StyleResolver.Apply() → Pattern match on value types            ││
│  └─────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────┘
```
...
### 3.2 StyleDeclaration Extensions
...
### 4.2 Box Model Expander (margin, padding, inset)
...
- [ ] Create `IStyleValue` interface
- [ ] Create `StyleValueType` enum
- [ ] Add interface to existing value types (Length, Color, Angle, Time, Frequency, Number, Percent)
...
- [ ] `VarValue` class
- [ ] `CalcValue` class
- [ ] `StyleValueList` class
- [ ] `StyleValueTuple` class
- [ ] Tests for each type

### Task 3: Property API (Priority: HIGH)
- [ ] Add `TypedValue` to Property base class
- [ ] Add `TryGetValue<T>()` method
- [ ] Bridge internal IPropertyValue to public IStyleValue
- [ ] StyleDeclaration extension methods
- [ ] Tests for property access

### Task 4: Shorthand Expansion (Priority: MEDIUM)
- [ ] `IShorthandExpander` interface
- [ ] `BoxModelExpander` (margin, padding, inset)
- [ ] `BorderExpander`
- [ ] `BorderRadiusExpander`
- [ ] `BackgroundExpander`
- [ ] `FlexExpander`
- [ ] `ShorthandExpanderRegistry`
- [ ] Tests for expansion

### Task 5: ComputedStyle (Priority: MEDIUM)
- [ ] `ComputedStyle` class
- [ ] Semantic query properties
- [ ] Integration with expanders
- [ ] Tests for computed style
...
src/ExCSS/
├── Values/
│   ├── IStyleValue.cs           # NEW - main interface
│   ├── IStyleValueMarkers.cs    # NEW - marker interfaces
│   ├── KeywordValue.cs          # NEW
│   ├── VarValue.cs              # NEW
│   ├── CalcValue.cs             # NEW
│   ├── StyleValueList.cs        # NEW
│   ├── StyleValueTuple.cs       # NEW
│   ├── RawValue.cs              # NEW - fallback for unparsed
│   └── (existing files - add IStyleValue)
├── Expansion/
│   ├── IShorthandExpander.cs    # NEW
│   ├── BoxModelExpander.cs      # NEW
│   ├── BorderExpander.cs        # NEW
│   ├── BorderRadiusExpander.cs  # NEW
│   └── ShorthandExpanderRegistry.cs # NEW
├── Computed/
│   └── ComputedStyle.cs         # NEW
└── Extensions/
    └── StyleDeclarationExtensions.cs # NEW
```
...
src/ExCSS/
├── Values/
│   ├── Length.cs       # Add : IStyleValue, IPrimitiveValue
│   ├── Color.cs        # Add : IStyleValue, IPrimitiveValue
│   ├── Angle.cs        # Add : IStyleValue, IPrimitiveValue
│   ├── Time.cs         # Add : IStyleValue, IPrimitiveValue
│   ├── Frequency.cs    # Add : IStyleValue, IPrimitiveValue
│   ├── Number.cs       # Add : IStyleValue, IPrimitiveValue
│   ├── Percent.cs      # Add : IStyleValue, IPrimitiveValue
│   └── Shadow.cs       # Add : IStyleValue, ICompositeValue
└── StyleProperties/
    └── Property.cs     # Add TypedValue accessor
```
...
1. `ICssValue` → `IStyleValue` interface pattern
2. `CssPeriodicValue` TRBL expansion logic → `BoxModelExpander`
3. `IValueAggregator` split/merge pattern → `IShorthandExpander`
...
1. ✅ `rule.Style["margin-top"].TypedValue` returns `Length(10, Unit.Px)` not string
2. ✅ `border: 1px solid red` expands to `border-width`, `border-style`, `border-color`
3. ✅ `var(--color)` returns `VarValue` with accessible variable name
