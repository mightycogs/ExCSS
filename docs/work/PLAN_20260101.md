...
- **25+ CSS shorthand property expansions** via `ShorthandRegistry`
- **Strongly-typed values**: `Color`, `Length`, `Percent`, `Number`, `Time`, `Angle`, `LinearGradient`, `RadialGradient`, `Shadow`, `CalcValue`, `VarValue`, `KeywordValue`
- **Property access**: `Property.TypedValue`, `Property.TryGetValue<T>()`
- **Shared helper**: `ExpanderHelpers.ExtractValues()` (no code duplication)
- **Test coverage**: 1511 tests passing
