...
| `BackgroundExpander` | `Expansion/BackgroundExpander.cs` |
| Tests: BackgroundExpander, TypedValue.TryGetValue | `ExCSS.Tests/Expansion/BackgroundExpanderTests.cs`, `ExCSS.Tests/PropertyTests/TypedValueTests.cs` |
| SourceLink opt-out for local/test builds | `ExCSS.csproj` |
...
### Task 3: Property API
- [x] Tests for `TryGetValue<T>()` method
...
- [x] **BackgroundExpander** - `background` → 8 longhands ✅
- [x] **GapExpander** - `gap` → `row-gap`, `column-gap` ✅
- [x] Unit tests for expanders (now 40+) ✅
...
### Task 4.4: BackgroundExpander (Status: DONE)

Implemented with defaults, multi-layer support, repeat-x/y expansion, origin/clip handling, position/size split. Tests cover repeat 2D, slash position/size, box defaults, global keywords, layer alignment.
